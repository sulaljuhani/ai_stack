...
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--> <div class="flex w-full"><div class="flex gap-1 scrollbar-none overflow-x-auto w-fit text-center text-sm font-medium rounded-full bg-transparent pt-1"><a${attr_class(`min-w-fit p-1.5 ${stringify(["/playground", "/playground/"].includes(store_get($$store_subs ??= {}, "$page", page).url.pathname) ? "" : "text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white")} transition`)} href="/playground">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Chat"))}</a> <a${attr_class(`min-w-fit p-1.5 ${stringify(store_get($$store_subs ??= {}, "$page", page).url.pathname.includes("/playground/completions") ? "" : "text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white")} transition`)} href="/playground/completions">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Completions"))}</a></div></div></div></nav> <div class="flex-1 max-h-full overflow-y-auto"><!--[-->`);
