"""Event Deletion Agent - Deletes events and recurring series."""
...
AGENT_CONFIG = get_agent_config("event_deleter")
AGENT_PROMPT = load_system_prompt(
    "event_deleter",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("event_deleter")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def event_deleter_node(state: MultiAgentState) -> Dict[str, Any]:
    """Delete events/series then hand off to validator. - direct execution without React loops."""
    logger.info("Event Deleter activated")
    try:
        ...
        response_content = f"Event Deleter executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="event_deleter",
        ...
        logger.error("Error in Event Deleter: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="event_deleter",
            context_key=CONTEXT_KEY,
