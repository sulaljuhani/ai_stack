"""Task Analyst - analyzes task data and productivity patterns."""
...
AGENT_CONFIG = get_agent_config("task_analyst")
AGENT_PROMPT = load_system_prompt(
    "task_analyst",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("task_analyst")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def task_analyst_node(state: MultiAgentState) -> Dict[str, Any]:
    """Analyze task data. - direct execution without React loops."""
    logger.info("Task Analyst activated")
    try:
        ...
        response_content = f"Task Analyst executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="task_analyst",
        ...
        logger.error("Error in Task Analyst: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="task_analyst",
            context_key=CONTEXT_KEY,
