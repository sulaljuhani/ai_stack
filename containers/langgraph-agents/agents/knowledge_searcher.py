"""Knowledge Search Agent - Vector/fuzzy/document searches (read-only)."""
...
async def knowledge_searcher_node(state: MultiAgentState) -> Dict[str, Any]:
    """Handle vector/fuzzy/doc searches (read-only)."""
    logger.info("Knowledge Searcher activated")
    try:
        agent = _get_agent()
        ...
        return {
            ...
            "current_agent": "knowledge_searcher",
        ...
        logger.error("Error in Knowledge Searcher: %s", e, exc_info=True)
        error_msg = AIMessage(content="I ran into an issue searching your knowledge base.")
        return {
            "messages": [error_msg],
            "current_agent": "knowledge_searcher",
            "previous_agent": state.get("current_agent"),
