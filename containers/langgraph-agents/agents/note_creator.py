"""Note Creation Agent - Creates new notes in the vault."""
...
AGENT_CONFIG = get_agent_config("note_creator")
AGENT_PROMPT = load_system_prompt(
    "note_creator",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("note_creator")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def note_creator_node(state: MultiAgentState) -> Dict[str, Any]:
    """Create notes then hand off to validator. - direct execution without React loops."""
    logger.info("Note Creator activated")
    try:
        ...
        response_content = f"Note Creator executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="note_creator",
        ...
        logger.error("Error in Note Creator: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="note_creator",
            context_key=CONTEXT_KEY,
