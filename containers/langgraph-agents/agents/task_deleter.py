"""Task Deletion Agent - Deletes or archives tasks with safeguards."""
...
AGENT_CONFIG = get_agent_config("task_deleter")
AGENT_PROMPT = load_system_prompt(
    "task_deleter",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("task_deleter")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def task_deleter_node(state: MultiAgentState) -> Dict[str, Any]:
    """Delete/archive tasks then hand off to validator. - direct execution without React loops."""
    logger.info("Task Deleter activated")
    try:
        ...
        response_content = f"Task Deleter executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="task_deleter",
        ...
        logger.error("Error in Task Deleter: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="task_deleter",
            context_key=CONTEXT_KEY,
