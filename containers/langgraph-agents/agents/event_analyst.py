"""Event Analyst - analyzes calendar/time usage patterns."""
...
AGENT_CONFIG = get_agent_config("event_analyst")
AGENT_PROMPT = load_system_prompt(
    "event_analyst",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("event_analyst")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def event_analyst_node(state: MultiAgentState) -> Dict[str, Any]:
    """Analyze events/time usage. - direct execution without React loops."""
    logger.info("Event Analyst activated")
    try:
        ...
        response_content = f"Event Analyst executed: {request[:50]}..."
        ...
        logger.error("Error in Event Analyst: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="event_analyst",
            context_key=CONTEXT_KEY,
