"""Reminder Deletion Agent - Deletes reminders with safety checks."""
...
AGENT_CONFIG = get_agent_config("reminder_deleter")
AGENT_PROMPT = load_system_prompt(
    "reminder_deleter",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("reminder_deleter")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def reminder_deleter_node(state: MultiAgentState) -> Dict[str, Any]:
    """Delete reminders and hand off to validator. - direct execution without React loops."""
    logger.info("Reminder Deleter activated")
    try:
        ...
        response_content = f"Reminder Deleter executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="reminder_deleter",
        ...
        logger.error("Error in Reminder Deleter: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="reminder_deleter",
            context_key=CONTEXT_KEY,
