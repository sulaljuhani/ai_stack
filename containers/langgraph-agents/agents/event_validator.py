"""Event Validation Agent - Verifies event operations."""
...
AGENT_CONFIG = get_agent_config("event_validator")
AGENT_PROMPT = load_system_prompt(
    "event_validator",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("event_validator")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def event_validator_node(state: MultiAgentState) -> Dict[str, Any]:
    """Validate event operations and record results. - direct execution without React loops."""
    logger.info("Event Validator activated")
    try:
        ...
        response_content = f"Event Validator executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="event_validator",
        ...
        logger.error("Error in Event Validator: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="event_validator",
            context_key=CONTEXT_KEY,
