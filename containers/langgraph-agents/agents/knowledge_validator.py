"""Knowledge Validation Agent - Verifies knowledge/note operations."""
...
AGENT_CONFIG = get_agent_config("knowledge_validator")
AGENT_PROMPT = load_system_prompt(
    "knowledge_validator",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("knowledge_validator")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def knowledge_validator_node(state: MultiAgentState) -> Dict[str, Any]:
    """Validate knowledge/note operations and record results. - direct execution without React loops."""
    logger.info("Knowledge Validator activated")
    try:
        ...
        response_content = f"Knowledge Validator executed: {request[:50]}..."
        ...
        logger.error("Error in Knowledge Validator: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="knowledge_validator",
            context_key=CONTEXT_KEY,
