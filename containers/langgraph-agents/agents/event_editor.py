"""Event Editor Agent - Updates events and recurring series."""
...
AGENT_CONFIG = get_agent_config("event_editor")
AGENT_PROMPT = load_system_prompt(
    "event_editor",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("event_editor")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def event_editor_node(state: MultiAgentState) -> Dict[str, Any]:
    """Edit events/series then hand off to validator. - direct execution without React loops."""
    logger.info("Event Editor activated")
    try:
        ...
        response_content = f"Event Editor executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="event_editor",
        ...
        logger.error("Error in Event Editor: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="event_editor",
            context_key=CONTEXT_KEY,
