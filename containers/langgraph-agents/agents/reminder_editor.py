"""Reminder Editor Agent - Updates existing reminders."""
...
AGENT_CONFIG = get_agent_config("reminder_editor")
AGENT_PROMPT = load_system_prompt(
    "reminder_editor",
    prompt_file=AGENT_CONFIG.prompt_file,
    partial_files=AGENT_CONFIG.partials,
)
AGENT_TOOLS = get_agent_tools("reminder_editor")
CONTEXT_KEY = AGENT_CONFIG.context_key


async def reminder_editor_node(state: MultiAgentState) -> Dict[str, Any]:
    """Edit reminders and hand off to validator. - direct execution without React loops."""
    logger.info("Reminder Editor activated")
    try:
        ...
        response_content = f"Reminder Editor executed: {request[:50]}..."

        # Internal-only response
        return create_internal_response(
            state=state,
            agent_name="reminder_editor",
        ...
        logger.error("Error in Reminder Editor: %s", e, exc_info=True)
        return create_internal_response(
            state=state,
            agent_name="reminder_editor",
            context_key=CONTEXT_KEY,
